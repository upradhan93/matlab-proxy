...
def validate_matlab_root_path(matlab_root: Path, is_custom_matlab_root: bool):
    ...
    Raises:
        MatlabInstallError
    """
    warn_string = f"Unable to find MATLAB installation at: {matlab_root}. "
    ...
    try:
        ...
        if is_custom_matlab_root:
            ...
            logger.debug(
                f"Based on matlab executable on system PATH, the resolved MATLAB root path:{matlab_root} exists, continuing to verify its validity..."
            )
    ...
    if not version_info_file_path.is_file():
        log_warn_string = (
            warn_string + f"Unable to locate {VERSION_INFO_FILE_NAME} at this location."
        )
