...
def get_process_startup_timeout():
    ...
    if custom_startup_timeout:
        if custom_startup_timeout.isdigit():
            custom_startup_timeout = int(custom_startup_timeout)
            logger.info(
            ...
            logger.warn(
                f"The value set for {mwi_env.get_env_name_process_startup_timeout()}:{custom_startup_timeout} is not a number. Using {constants.__PROCESS_START_TIMEOUT} as the default value"
            )
